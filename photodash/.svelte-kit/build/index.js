...
export class Server {
	...
	async init({ env }) {
		...
		const prv = Object.fromEntries(entries.filter(([k]) => !k.startsWith('SUPERVISOR_')));

		const pub = Object.fromEntries(entries.filter(([k]) => k.startsWith('SUPERVISOR_')));
