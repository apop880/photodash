...
<label class="label" for="icon">
  ...
  <input name="serviceType" type="text" id="serviceType" class="input input-bordered w-full" value={data?.serviceType ?? ''} />
  <label class="label" for="target">
      <span class="label-text">Target</span>
  </label>
  <input name="target" type="text" id="target" class="input input-bordered w-full" value={data?.target ?? ''} />
